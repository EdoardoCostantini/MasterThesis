...
draw_PsiInv_InvWish = function(n,bMat,B0=diag(2)){
  ...
  PsiInvDraw = rwish(v=n+2,S=solve(ScaleMatrix))
...
draw_PsiInv_MSBeta2 = function(yvec,Xmat,Zi,wvec,beta,bMat,PsiInv,phivec,Omega,B0Inv){
  ScaleMatrix = t(bMat)%*%bMat+Omega
  PsiInvDraw = rwish(v=n+2,S=solve(ScaleMatrix))
  ScaleOmega = solve(PsiInvDraw+B0Inv)
  Omega = rwish(v=4,S=ScaleOmega)
...
draw_PsiInv_HW = function(PsiInv,avec,bMat){
  ...
  Ak = 10**5
  ScaleMatrix = t(bMat)%*%bMat+4*diag(1/avec)
  PsiInvDraw = rwish(v=n+3,S=solve(ScaleMatrix))
  scale_avec = 2*diag(PsiInvDraw) + 1/Ak**2
  avec = rinvgamma(2,shape=2,scale=scale_avec)
...
compute_weight = function(Xmat,Zi,wvec,beta,bMat){
  ...
  s2tilde = pi**2*(nu1-2)/(3*nu1)
  muvec = c(Xmat%*%beta+c(Zi%*%t(bMat)))
  weight = exp(sum(dlogis(wvec,location=muvec,log=T)-dt((wvec-muvec)/sqrt(s2tilde),df=nu1,log=T)+.5*log(s2tilde)))
  return(weight)
}
...
MCMC_mixlogreg_SBeta2_A = function(yvec,n,J,B0=1,samsize=1e3){
  ...
  #note W_i and Z_i are equal for all i in this setup
  Rstar = solve(t(Zi*weightVec[1:J])%*%Zi)
  if(is.matrix(B0)==F){#B0==1, then use default prior scale of Kass&Nat'06, B0==2, then set B0 equal to the ML estimate
    if(B0==1){
      B0 = Rstar
...
out2 = MCMC_mixlogreg_SBeta2_A(yvec,n,J,B0 = 1e3*diag(2),samsize=samsize)
out3 = MCMC_mixlogreg_InvWish_A(yvec,n,J,B0 = diag(2),samsize=samsize)
out4 = MCMC_mixlogreg_SBeta2_A(yvec,n,J,B0 = 1,samsize=samsize)
out5 = MCMC_mixlogreg_InvWish_A(yvec,n,J,B0 = 1,samsize=samsize)
out6 = MCMC_mixlogreg_SBeta2_A(yvec,n,J,B0 = 2,samsize=samsize)
out7 = MCMC_mixlogreg_HW_A(yvec,n,J,samsize=samsize)
...
#  pr = 2
  out1 = MCMC_mixlogreg_SBeta2_A(yvec,n,J,B0 = 1,samsize=samsize)
  pr = 3
