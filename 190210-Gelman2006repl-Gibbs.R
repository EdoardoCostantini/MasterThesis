...
# 1. Data ####
  data("schools")
  J <- nrow(schools)
  nj <- 50 # I couldnt find it, but it's written more than 30 per school
  N <- J*nj
  y <- schools$estimate
...
# For details see book on page 289 and appendix C
  ...
  # Full conditional Posterior of mu (given uniform prior on mu)
    ...
    mu_update <- function (){
      vmu <- 1/(J/tau**2+1/g20)
      emu <- vmu*(J*mean(theta)/tau**2 + mu0/g20)
      mu <- rnorm(1,
                  emu,
                  sqrt(vmu))
    }
...
# 3. gibss Sampler ####
  # Prior specification
    ...
    t20_vec  <- c(0, 1, 1)
    # Prior for mu
    mu0<- 0 ; g20 <- 1e-6 # to match the OpenBUGS model
